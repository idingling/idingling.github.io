...
if ($output['status'] == 1) {
    ...
    if (empty($old_sppasswd)) {
        ...
        //提交版块sitemap
        $sitemapbolckreturnjson = baidu_submit_sitemap_index('add', 3, $siteurl, $sppasswd, $sign);
        $sitemapResult = json_decode($sitemapbolckreturnjson['json'], true);
        if (!isset($sitemapResult['status'])) {
            baidu_show_json(array(
                'error' => 1,
                'msg' => diconv(lang('plugin/baidusubmit', 'authfailed') . "[URL:{$sitemapbolckreturnjson['url']}]", CHARSET, 'utf-8'),
            ));
        }
        $result += (int) $sitemapResult['status'];
        ...
        if ($result === 0) {
            ...
            if ($old_sppasswd) {
                ...
                baidu_submit_sitemap_index('del', 3, $siteurl, $old_sppasswd, $sign);
                baidu_submit_sitemap_index('del', 4, $siteurl, $old_sppasswd, $sign);
                baidu_submit_sitemap_index('del', 5, $siteurl, $old_sppasswd, $sign);
            }
            baidu_show_json(array(
                'error' => 0,
