	...
	<!--{/if}-->
					...
					<tbody>
						...
						<tr>
							...
							<td class="icn" style="text-align:center;"><img src="{IMGDIR}/ann_icon.gif" alt="{lang announcement}" /></td>

							<!--{if $_G['forum']['ismoderator'] && !$_GET['archiveid']}--><td class="o cl">&nbsp;</td><!--{/if}-->

							<th class="xs2">{lang announcement}: <!--{if empty($announcement['type'])}--><a href="forum.php?mod=announcement&id=$announcement[id]#$announcement[id]" target="_blank">$announcement[subject]</a><!--{else}--><a href="$announcement[message]" target="_blank">$announcement[subject]</a><!--{/if}--></th>
...
<div class="th showmenubox">
        ...
        <div class="y">
            ...
            <a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=hot" class="{if $_GET['filter'] == 'digest'} xw1{/if}">{lang hot_thread}</a>&nbsp;
            ...
            <a {if empty($_G['cookie']['forumdefstyle'])} href="forum.php?mod=forumdisplay&fid=$_G[fid]&forumdefstyle=yes" class="chked"{else} href="forum.php?mod=forumdisplay&fid=$_G[fid]&forumdefstyle=no" class="unchk"{/if} title="{lang view_thread_imagemode}{lang view_thread}">{lang view_thread_imagemode}</a>  <span class="pipe">|</span>

            <!--{/if}--> 

            <span id="atarget" {if $_G['cookie']['atarget'] > 0}onclick="setatarget(-1)" class="atarget_1"{else}onclick="setatarget(1)"{/if} title="{lang new_window_thread}">{lang new_window}</span> 
        ...
        <span class="showmenu_outer">
            ...
            <a id="filter_special" href="javascript:;" class="showmenu {if $_GET['specialtype']} xw1{/if}" onclick="showMenu(this.id)">

                <!--{if $_GET['specialtype'] == 'poll'}-->

                {lang thread_poll}

                <!--{elseif $_GET['specialtype'] == 'trade'}-->

                {lang thread_trade}

                <!--{elseif $_GET['specialtype'] == 'reward'}-->

                {lang thread_reward}

                <!--{elseif $_GET['specialtype'] == 'activity'}-->

                {lang thread_activity}

                <!--{elseif $_GET['specialtype'] == 'debate'}-->
        ...
        <span class="showmenu_outer">
            ...
            <a id="filter_orderby" href="javascript:;" class="showmenu {if $_GET['specialtype']} xw1{/if}" onclick="showMenu(this.id)">
                ...
                <!--{elseif $_GET['orderby'] == 'heats'}-->

                {lang order_heats}

                <!--{else}-->

                {lang list_default_sort}
		...
		<form method="post" autocomplete="off" name="moderate" id="moderate" action="forum.php?mod=topicadmin&action=moderate&fid=$_G[fid]&infloat=yes&nopost=yes">
			...
			<table summary="forum_$_G[fid]" cellspacing="0" cellpadding="0" id="threadlisttableid">
				...
				<!--{if !$separatepos || !$_G['setting']['forumseparator']}-->
					...
					<tbody id="separatorline" class="emptb"><tr><td class="icn"></td><!--{if !$_GET['archiveid'] && $_G['forum']['ismoderator']}--><td class="o cl"></td><!--{/if}--><th></th><!--{if CURMODULE == 'guide'}--><td class="by"></td><!--{/if}--><td class="by"></td><td class="num"></td><td class="by"></td></tr></tbody>
						...
						<!--{loop $_G['forum_threadlist'] $key $thread}-->
							...
							<!--{if $_G[setting][forumseparator] == 1 && $separatepos == $key + 1}-->
								...
								<tbody id="separatorline">
									...
									<tr class="ts" style=" height: 30px; line-height: 30px;">
										...
										<th style="border-right: 1px solid #E8E8E8;"><!--{if empty($_G['forum']['picstyle']) && $_GET['orderby'] == 'lastpost' && !$_GET['filter']}--><a href="javascript:;" onclick="checkForumnew_btn('{$_G['fid']}')" title="{lang showupgrade}" class="forumrefresh">{lang forum_thread}</a><!--{else}-->&nbsp;<!--{/if}--></th>
							...
							<tbody id="$thread[id]">
								...
								<tr>
                                    ...
                                    <div class="forumava <!--{if $thread[folder] == 'lock' || $thread['special'] || $thread[displayorder]}-->s_lnoavatar<!--{/if}-->"> 
									...
									<th class="$thread[folder] forumtit">
										...
										<!--{if $thread['attachment'] == 2}-->
											...
											<img src="$_G['style'][styleimgdir]/time_common/image_s.gif" alt="attach_img" title="{lang attach_img}" align="absmiddle" />

										<!--{elseif $thread['attachment'] == 1}-->

											<img src="$_G['style'][styleimgdir]/time_common/common.gif" alt="attachment" title="{lang attachment}" align="absmiddle" />
										...
										<!--{if $thread['digest'] > 0 && $filter != 'digest'}-->
											...
											<img src="$_G['style'][styleimgdir]/time_common/digest_$thread[digest].png" align="absmiddle" alt="digest" title="{lang thread_digest} $thread[digest]" />
										...
										<!--{if $thread['displayorder'] == 0}-->
											...
											<!--{if $thread[recommendicon] && $filter != 'recommend'}-->
												...
												<img src="$_G['style'][styleimgdir]/time_common/recommend_$thread[recommendicon].gif" align="absmiddle" alt="recommend" title="{lang thread_recommend} $thread[recommends]" />

											<!--{/if}-->

											<!--{if $thread[heatlevel]}-->

												<img src="$_G['style'][styleimgdir]/time_common/hot_$thread[heatlevel].gif" align="absmiddle" alt="heatlevel" title="{lang heats}: {$thread[heats]}" />

											<!--{/if}-->

											<!--{if $thread['rate'] > 0}-->

												<img src="$_G['style'][styleimgdir]/time_common/agree.gif" align="absmiddle" alt="agree" title="{lang rate_credit_add}" />

											<!--{elseif $thread['rate'] < 0}-->

												<img src="$_G['style'][styleimgdir]/time_common/disagree.gif" align="absmiddle" alt="disagree" title="{lang posts_deducted}" />
										...
										<!--{if $thread[folder] == 'lock'}-->
											...
											<img src="$_G['style'][styleimgdir]/time_common/folder_lock.gif" />

										<!--{elseif $thread['special'] == 1}-->

											<img src="$_G['style'][styleimgdir]/time_common/pollsmall.gif" alt="{lang thread_poll}" />

										<!--{elseif $thread['special'] == 2}-->

											<img src="$_G['style'][styleimgdir]/time_common/tradesmall.gif" alt="{lang thread_trade}" />

										<!--{elseif $thread['special'] == 3}-->

											<img src="$_G['style'][styleimgdir]/time_common/rewardsmall.gif" alt="{lang thread_reward}" />

										<!--{elseif $thread['special'] == 4}-->

											<img src="$_G['style'][styleimgdir]/time_common/activitysmall.gif" alt="{lang thread_activity}" />

										<!--{elseif $thread['special'] == 5}-->

											<img src="$_G['style'][styleimgdir]/time_common/debatesmall.gif" alt="{lang thread_debate}" />

										<!--{elseif in_array($thread['displayorder'], array(1, 2, 3, 4))}-->

											<img src="$_G['style'][styleimgdir]/time_common/pin_$thread[displayorder].gif" alt="$_G[setting][threadsticky][3-$thread[displayorder]]" />

										<!--{else}-->

											<img src="$_G['style'][styleimgdir]/time_common/folder_$thread[folder].gif" />
                                        ...
                                        楼主:<a href="home.php?mod=space&uid=$thread[authorid]" c="1">

                                      <span>$thread[author]</span></a><span>$thread[dateline]</span><span class="pipe">|</span>
                                        ...
                                        <span class="ico_reply">$thread[allreplies]</span>

                                       <span class="ico_see"><!--{if $thread['isgroup'] != 1}-->$thread[views]<!--{else}-->{$groupnames[$thread[tid]][views]}<!--{/if}--></span>
							...
							<li style="width:{$waterfallwidth}px;">
								...
								<div class="auth cl">
									...
									<cite class="xg1 y">
										...
										人气: <!--{if $thread['isgroup'] != 1}-->$thread[views]<!--{else}-->0<!--{/if}-->
...
<!--{if !IS_ROBOT}-->
	...
	<div id="filter_special_menu" class="p_pop showsub" style="display:none" change="location.href='forum.php?mod=forumdisplay&fid=$_G[fid]&filter='+$('filter_special').value">
