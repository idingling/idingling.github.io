<?php echo 'Theme by Time  http://www.xdmb.net';exit;?>
<!--{eval
...
<td class="pls pls_relative" rowspan="2"><!--{if $post['first']}-->
			...
			<!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}-->
				...
				<!--{if $post['avatar'] && $showavatars}-->
					...
					<div class="avatar"{if !($_G['setting']['threadguestlite'] && !$_G['uid'])} onmouseover="showauthor(this, 'userinfo$post[pid]')"{/If}><a href="home.php?mod=space&uid=$post[authorid]" class="avtm" target="_blank">$post[avatar]</a></div>
                    				...
                    				<!--{elseif !$postshowavatars}-->
					<div class="avatar">{lang member_avatar_banned}</div>
                    
                    
				<!--{/if}-->
                <div class="avt_name">
						<div class="authi"><a href="home.php?mod=space&uid=$post['authorid']" target="_blank" class="xs2"{if $post[groupcolor]} style="color: $post[groupcolor]"{/if}>$post[author]</a>$authorverifys</div>
					</div>
    ...
    <i class="arr"></i>
		<div class="pi <!--{if $post['first']}-->tpi<!--{/if}-->"<!--{if $close_leftinfo || (!$post['first'] && !$post['anonymous'] && $postshowavatars && $showavatars)}--> style="height: 44px;"<!--{/if}-->>
			<!--{if !IS_ROBOT}-->
				<!--{if !$postcount && !$_G['forum_thread']['archiveid'] && $post['first'] }-->
					<div id="fj" class="y" style="margin: 13px 0 0 8px;">
						<label class="z" style="color: #999999; font-size: 14px;">{lang thread_redirect_postno}</label>
			...
			<div class="pti" style="color: #515151;">
                ...
                <!--{if $post['first']}-->
               <span class="ico_see">$_G[forum_thread][views]</span><span class="ico_reply">$_G[forum_thread][allreplies]</span>
                <!--{/if}-->


					<a href="home.php?mod=space&uid=$post[authorid]" target="_blank" class="xi2 z inside_avt" style="padding-right:10px;">{echo avatar($post['authorid'], 'small')}</a>
				...
				<!--{if $post['authorid'] && !$post['anonymous']}-->
					<!--{if $_self}-->
					<a style="color: #555555; font-size: 14px; font-weight: bold;">&nbsp;{lang thread_author}</a><span class="pipe">|</span>
					<!--{/if}-->
					<!--{if !$_G['setting']['authoronleft']}--><a href="home.php?mod=space&uid=$post[authorid]" target="_blank" class="xi2" >
                    $post[author]</a>$authorverifys<!--{/if}-->
					<!--{if $close_leftinfo}--><!--{/if}-->
					<em id="authorposton$post[pid]"><a href="home.php?mod=space&uid=$post['authorid']" target="_blank" style="padding: 0 5px 0 0; color: #555555; font-size: 14px; font-weight: bold;">$post[author]</a> {lang poston} $post[dateline]</em>
		...
		<div class="pct" style="color: #515151;">
		...
		<!--{if helper_access::check_module('collection') && !$_G['forum']['disablecollect']}-->
			<!--{if $post['relatecollection']}-->
				<div class="time_cm">
					<h3></span>{lang collection_related}</h3>
					<ul class="rate">
	...
	<td class="plc" style="overflow:visible;{if $close_leftinfo}--> width:100%{/if}">
		<div class="po bbd hin">
			<!--{if !$post['first'] && $modmenu['post']}-->
				<span class="y" style="color: #999999;">
			...
			<div class="pob cl">
                ...
                <!--{if $post['invisible'] == 0}-->
						...
						<!--{/if}-->
						<!--{if $post['authorid'] != $_G['uid']}-->
							<a href="javascript:;" onclick="showWindow('miscreport$post[pid]', 'misc.php?mod=report&rtype=post&rid=$post[pid]&tid=$_G[tid]&fid=$_G[fid]', 'get', -1);return false;" style="padding-left: 0;">{lang report}</a>
						<!--{/if}-->
					<!--{/if}-->
